...
public:
    vector<vector<int>> closestNodes(TreeNode* root, vector<int>& queries) {
        ...
        // printf("hello\n");
		// fflush(stdout);
        // for(const auto&i:OrderedTreeNodes)
        	// printf("%d ", i->val);
        // fflush(stdout);
        int NoOfNodesInTree = OrderedTreeNodes.size();
        for(const auto &i:queries)
        {
        	int LowerBound = BinarySearch(0, NoOfNodesInTree-1, i);
        	// printf("i: %d; LowerBound: %d\n", i, LowerBound);
        	// fflush(stdout);
        	int UpperBound = LowerBound +1;
        	if(LowerBound == -1)
        		res.push_back(vector<int>{-1, OrderedTreeNodes[0]->val});
        	else if(OrderedTreeNodes[LowerBound]->val == i)
        		res.push_back(vector<int>{i,i});
        	else if(LowerBound == NoOfNodesInTree-1)
        		res.push_back(vector<int>{OrderedTreeNodes[NoOfNodesInTree-1]->val, -1});
        	else
        		res.push_back(vector<int>{OrderedTreeNodes[LowerBound]->val, OrderedTreeNodes[UpperBound]->val});
        }
        return(res);
...
private:
	vector<TreeNode*> OrderedTreeNodes;
	int BinarySearch(int LowerBound, int UpperBound,int GivenNumber){
		
		while(LowerBound<=UpperBound){
			int Mid = (LowerBound + UpperBound)/2;
			if(OrderedTreeNodes[Mid]->val > GivenNumber)
				UpperBound = Mid -1;
			else if(OrderedTreeNodes[Mid]->val < GivenNumber)
				LowerBound = Mid +1;
			else
				return(Mid);
		}
		//in the end, Upper bound = Lower Bound -1, and UpperBound+1 satisfy that val >GIven number, LowerBound -1 satisfy that val < Given number
		return(UpperBound);
	}
	...
	void DFS(TreeNode* root){
        if(root!=nullptr){
        	DFS(root->left);
        	OrderedTreeNodes.push_back(root);
        	DFS(root->right);
        }
        else{
        	return;
...
int main(){
	...
	vector<string> input={"19","3","20","2","10","null","null","1","null","5","15","null","null","4","6"};
	TreeNode* root = ArrayToBinaryTree(input);
	vector<int> queries ={143265,20,19,172253,562096,330190,474166,460360,929962,2};
	vector<vector<int>> res = mysol.closestNodes(root, queries);
