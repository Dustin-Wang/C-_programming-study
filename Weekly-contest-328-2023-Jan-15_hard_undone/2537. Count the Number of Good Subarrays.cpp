...
public:
    long long countGood(vector<int>& nums, int k) {
        int numsNos = nums.size();
        //consider the subarrays that end at location end
        int start = 0;
        long long res = 0;
        map<int,int> NosOfNumbers;
        int GoodPairsBetweenStartAndEnd = 0;
        for(int end=0; end<numsNos; end++){
        	//now let us define a variable start, start satisfies that [start, end] is the first subarray 
        	//whose good pairs that are <k
        	//then we need to have a map to record the Nos of numbers between start and end;

        	//insert the nums[end] into the map
        	if(NosOfNumbers.find(nums[end]) == NosOfNumbers.end())
        		NosOfNumbers[nums[end]] = 1;
        	else{
        		//now we need to count the good pairs between the start and new end;
        		GoodPairsBetweenStartAndEnd += NosOfNumbers[nums[end]];
        		NosOfNumbers[nums[end]]++;
        	}
        	while(GoodPairsBetweenStartAndEnd >= k)
        	{
        		//now we need to move our start to be the first subarray whose good pairs that are <k
        		
        		NosOfNumbers[nums[start]]--;
        		GoodPairsBetweenStartAndEnd -= NosOfNumbers[nums[start]];
        		start++;
        	}
        	// printf("start: %d, end: %d; GoodPairsBetweenStartAndEnd: %d\n", start, end, GoodPairsBetweenStartAndEnd);
        	res += start;
        }
        return res;
    }
...
int main(){
	...
	// }
  	...
  	int k = 2;
  	cout<<mysol.countGood(nums, k)<<endl;
 	return(0);	
}