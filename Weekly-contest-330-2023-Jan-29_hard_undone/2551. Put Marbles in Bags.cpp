...
public:
    long long putMarbles(vector<int>& weights, int k) {
    	...
    	//for maximum score, the key is to start from weights with bags number = weightsNo
    	long long MaximumScoreOutput = 0;
    	for(auto weight:weights)
    		MaximumScoreOutput += weight*2;
    	
    	// printf("MaximumScoreOutput: %lu\n", MaximumScoreOutput);
    	//now we want to get the maximum scores when we need to divide the weights into k bags
    	//the key is to remove the partitions in the weights.
    	//thre are weightsNo-1 partitions, each time we remove one, the score will be reduced by
    	//weight[i]+weight[i+1];

    	vector<int> CostForEachPartition;
    	for(int i=0; i< weightsNo-1; i++)
    		CostForEachPartition.push_back(weights[i]+weights[i+1]);

    	sort(CostForEachPartition.begin(), CostForEachPartition.end());
    	// printvector(CostForEachPartition);
    	for(int l=weightsNo-1; l>k-1; l--)//l is the number of partitions that we need here.
    		MaximumScoreOutput -= CostForEachPartition[weightsNo-1-l];
    	// printf("MaximumScoreOutput: %lu\n", MaximumScoreOutput);
    	//for minimum score, the key is to start from weights with bags number = 0
    	// then we keep adding the partition whose score is the smallest

    	long long MinimumScoreOutput = weights[0]+weights[weightsNo-1];
    	for(int l=0; l<k-1; l++)
    		MinimumScoreOutput +=CostForEachPartition[l];
