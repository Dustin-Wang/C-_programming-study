...
public:
    int colorTheGrid(int m, int n) {
    	...
    	for(const auto&c:NeighborsForColumns){
    		res=(res+CountColorTheGrid(m,n-1,c.first,NeighborsForColumns))%mod;
    	}
    	return res;
        
    }
private:
	int CountColorTheGrid(int m,int n,string CurrentCol,
		unordered_map<string, vector<string>>& NeighborsForColumns){
		if(n==0){
			return 1;
		}
		int res=0;
		for(const auto &d:NeighborsForColumns[CurrentCol]){
			res=(res+CountColorTheGrid(m,n-1,d,NeighborsForColumns))%mod;
		}
		return res;
	}
