...
public:
    long long maxStrength(vector<int>& nums) {
        long long res = 1;
        long long MaximumNegative = LLONG_MIN;
        vector<int> Buckets(19,0);//index-9 is the value
        for(int i=0;i<nums.size(); i++)
        	Buckets[nums[i]+9]++;
        // printvector(Buckets);
        int PositivStudents = 0;
        int NegativeStudents = 0;

        for(int j=-9; j<=9; j++)
        {
        	if(Buckets[j+9]>0 && j !=0 ){
        		res = res*pow(j, Buckets[j+9]);
        		if(j<0 && MaximumNegative < j)
        			MaximumNegative = j;
        		if(j<0)
        			NegativeStudents += Buckets[j+9];
        		if(j>0)
        			PositivStudents += Buckets[j+9];
        	}
        	
        }
        // cout<<res<<endl;
        // cout<<MaximumNegative<<endl;
        if(res<0)
        {
        	if(NegativeStudents+PositivStudents>1)
        		res = res/MaximumNegative;
        	else
        	{
        		if(Buckets[9]>0)
        			res =  0;
        ...
        {
        	if(NegativeStudents+PositivStudents==0)
        		res = 0;
        }
        return(res);
    }
...
int main(){
	Solution mysol;
	// vector<int> s{3,-1,-5,2,5,-9};	
	// vector<int> s{-4,-5,-4};	
	// vector<int> s{0,-1};
	// vector<int> s{8,6,0,5,-4,-8,-4,9,-1,6,-4,8,-5};
	vector<int> s{-9};

	cout<<mysol.maxStrength(s)<<endl;
