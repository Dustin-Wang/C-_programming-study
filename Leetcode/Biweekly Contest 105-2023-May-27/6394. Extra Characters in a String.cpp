...
public:
    int minExtraChar(string s, vector<string>& dictionary) {
        ...
        for(auto word:dictionary)
        	dic->insert(word);
        int sLen = s.size();
        int res = minExtraChar2(s, 0, sLen);
        return(res);
    }
    int minExtraChar2(string s, int start, int sLen)
    {	
    	if(start >= sLen)
    		return(0);
    	else
    	{
    		...
    		// int AlreadyExtra = 0;
    		int i = start;
    		int UpperBoundFori = sLen-1;
    		while(i<=UpperBoundFori && i-start<res)
    		{
    			string prefix = "";
    			...
    			while(j<sLen && dic->startsWith(prefix))
        		{
        			if(dic->search(prefix))
        			{	
        				//we found a substring that is in dictionary
        				// cout<<"prefix4: "<<prefix<<endl;
        				// cout<<"j+1: "<<j+1<<endl;
        				int recur = minExtraChar2(s, j+1, sLen);//suppose we took this substring, and then check the rest by recursion.

        				// cout<<"recur:"<<recur<<endl;
        				res = min(res, recur + i-start);
        				UpperBoundFori = j+1;
        				// cout<<"res:"<<res<<endl;
        			}	
        			j++;
        		...
        		// cout<<"prefix3: "<<prefix<<endl;
        		//at this moment, //string[i:j] is no longer a prefix in dic, but string[i:j-1] is a prefix in dic 
        		// AlreadyExtra += max(j,i+1)-i;
        		// cout<<"AlreadyExtra: "<<AlreadyExtra<<endl;
        		i=i+1;
    		}
    		return(res);
    	}
...
int main(){
	...
	// string s ="rukalyycfehuwrlbdeyatxhgyzhsxwdzqbdenrdtfw";
	// string s= "qbden";
	// vector<string> dictionary = {"koemv","z","ycexl","dzq","rdt","w","itlypj","deyatx","zhsb","iq","a","bew","bwm","ly","nf","jcqi","vqmdze","flilnf","uka","ciz","uqiq","hdfyax","uwrlb","xoje","zezw","en","szdbes","hgyzhs","vedvqc","ypbml"};
	string s="okwqrototydnjmtauzoaefzttauzoabjazakpovaxlusbx";
vector<string> dictionary = {"r","pzvnt","yxgtfy","zaqj","vdd","iqq","vrbppx","djmo","u","toqs","dxlk","oxf","wofej","zuvoo","bjazak","stnmn","zbj","okw","x","z","xdhs","eszgo","f","h","oa","xfql","rxuneu","v","ihct","rsps","chb","m","vpvaa","ngn","mm","vtuuae","o","a","iwh","pmnqm","vaxl","tauzo","iijwds","idfee","gyq"};
	cout<<mysol.minExtraChar(s, dictionary)<<endl;
