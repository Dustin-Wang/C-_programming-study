...
public:
    vector<int> getSubarrayBeauty(vector<int>& nums, int k, int x) {
        ...
        if(Negatives.size()<x)
        	...
        	res.push_back(Negatives[x-1]);
        for(int l=k; l< numsLen; l++)
        {
        	//pop out the nums[l-1] in the negatives
        	if(nums[l-k]<0){
        		auto IndexLminus1= lower_bound(Negatives.begin(), Negatives.end(), nums[l-k]);
        		Negatives.erase(IndexLminus1);
        	}
        	if(nums[l]<0){
        		auto indexL = lower_bound(Negatives.begin(), Negatives.end(), nums[l]);
        		Negatives.insert(indexL, nums[l]);
