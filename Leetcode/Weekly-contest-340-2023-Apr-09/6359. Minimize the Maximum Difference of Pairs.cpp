...
public:
    int minimizeMax(vector<int>& nums, int p) {
        ...
        printvector(nums);
        int LB = 0;
        int UB = nums[numsLen-1]-nums[0];
        // printf("LB: %d; UB: %d\n", LB, UB);
        while(LB <= UB)
        {
        	// printf("LB: %d; UB: %d\n", LB, UB);
        	int mid = (LB + UB)/2;
        	//now we check whether this mid is acheivable or not
        	//let us start from smallest value, and check how many consecutive pairs got pair adjacent distance <= mid
        	int AvailablePairs = 0;
        	// int InConsecutiveBlock=0;
        	int CountOfThisConsecutiveBlock = 0;
        	for(int i=0; i< numsLen-1; i++)
        	{

        		if(CountOfThisConsecutiveBlock==0 && nums[i+1]-nums[i] <= mid)
        		{	
        			// InConsecutiveBlock = 1;
        			while( i< numsLen-1 && nums[i+1]-nums[i] <= mid ){
        				CountOfThisConsecutiveBlock++;
        				i++;
        			}
        			if(CountOfThisConsecutiveBlock%2)
        				AvailablePairs += (CountOfThisConsecutiveBlock/2+1);
        			else
        				AvailablePairs += CountOfThisConsecutiveBlock/2;

        			CountOfThisConsecutiveBlock = 0;
        		}
        	}
        	if(AvailablePairs >= p)
        		UB = mid-1;
...
int main(){
	...
	vector<int> nums ={4,2,1,2};int p=1;
	// vector<int> nums ={0,5,3};
	cout<<mysol.minimizeMax(nums,p)<<endl;
