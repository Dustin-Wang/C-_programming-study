...
public:
    int minIncrements(int n, vector<int>& cost) {
     	...
     	int Depths = floor(log(n+1)/log(2));//say 3
     	// cout<<Depths<<endl;
     	int NoPath = 1<< (Depths-1);//2^2 = 4;
     	// vector<int> WeightsForPath{cost[0]};

     	// for(int i=1; i<Depths;i++)
     	// {
     	// 	vector<int> NewWeightsForPath;
     	// 	int index = (1<<i);
     	// 	for(int j=0; j<WeightsForPath.size(); j++)
     	// 	{
     	// 		NewWeightsForPath.push_back(WeightsForPath[j]+cost[index-1]);
     	// 		index++;
     	// 		NewWeightsForPath.push_back(WeightsForPath[j]+cost[index-1]);
     	// 		index++;
     	// 	}
     	// 	WeightsForPath = NewWeightsForPath;
     	// }
     	// vector<vector<int>> WeightsForPathWithIndex;
     	// for(int i=0; i<WeightsForPath.size(); i++)
     	// 	WeightsForPathWithIndex(vector<int>{WeightsForPath[i],i});

     	// sort(WeightsForPathWithIndex.begin(), WeightsForPathWithIndex.end(),greater<int>());
     	
     	// int MaximumWeight = WeightsForPathWithIndex[0][0];
     	// int MaximumWeightPathIndex = WeightsForPathWithIndex[0][1];
     	int res = 0;

     	// int MaximumPathNode = MaximumWeightPathIndex;

     	vector<int> AccumulativeWeightsForParentsFromLeaf(NoPath,0);
     	int t = Depths-1;
     	do
     	{
     		vector<int> NewAccumulativeWeightsForParentsFromLeaf;
     		int l=(1<<t);
     		while(l< (1<<(t+1)) )
     		{
     			int maxofTwo = 0;
     			// printvector(AccumulativeWeightsForParentsFromLeaf);
     			// cout<<"l-(1<<t):"<<l-(1<<t)<<endl;

     			int left = cost[l-1]+AccumulativeWeightsForParentsFromLeaf[l-(1<<t)];
     			int right = cost[l]+AccumulativeWeightsForParentsFromLeaf[l-(1<<t)+1];

     			if(left>right){
     				maxofTwo = left;
     				NewAccumulativeWeightsForParentsFromLeaf.push_back(left);
     			}
     			else{
     				maxofTwo = right;
     				NewAccumulativeWeightsForParentsFromLeaf.push_back(right);
     			}
     			res= res+(maxofTwo-left) + (maxofTwo-right);

     			// cout<<"l:"<<l<<":"<<maxofTwo<<endl;
     			l=l+2;
     		}
     		AccumulativeWeightsForParentsFromLeaf = NewAccumulativeWeightsForParentsFromLeaf;
     		t--;
...
int main(){
	Solution mysol;
	// int inputs[][2]={{0,2},{1,2},{3,1},{1,1},{2,1}};
	// int n=4;
	// int inputs[][2]={{0,100000}};
	// int n=1;
	// vector<vector<int>> inputs2 = ArrayTo2dVector(inputs);
	// auto res = mysol.colorTheArray(n, inputs2);
	int n= 7;
	vector<int> cost{1,5,1000,2,3,3,1};
	
	cout<<mysol.minIncrements(n, cost)<<endl;
