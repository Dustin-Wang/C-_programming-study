...
public:
    vector<int> distinctDifferenceArray(vector<int>& nums) {
     	int NoNums = nums.size();
     	// vector<int> DistinctCollection(50, 0);//1 to 50
     	vector<int> DistinctRecordPrefix(NoNums,0);
     	vector<int> DistinctRecordSuffix(NoNums,0);
     	vector<int> NumsExists(50,0);
     	vector<int> res;

     	int count = 0;
     	for(int i=0; i<NoNums; i++)
     	{
     		if(NumsExists[nums[i]-1]==0)
     		{
     			count++;
     			NumsExists[nums[i]-1]=1;
     		}
     		DistinctRecordPrefix[i]=count;
     	}
     	//printvector(DistinctRecordPrefix);
     	count = 0;
     	NumsExists = vector<int>(50,0);
     	for(int i=NoNums-1; i>=1; i--)
     	{
     		if(NumsExists[nums[i]-1]==0)
     		{
     			count++;
     		...
     		DistinctRecordSuffix[i-1]= count;
     	}
     	//printvector(DistinctRecordSuffix);
     	for(int i=0; i<NoNums; i++)
     		res.push_back(DistinctRecordPrefix[i]-DistinctRecordSuffix[i]);
     	return(res);
    }
private:	
...
int main(){
	...
	// int inputs[][5]={{1,2,3,3,2},{3,4,4,5,1}};
	// vector<int> start{1,1};
	// vector<int> target{4,5};
	// int inputs[][5]={{3,2,3,4,4},{3,3,5,5,5},{3,4,5,6,6}};
	// vector<int> start{3,2};
	// vector<int> target{5,7};
	vector<int> res{3,2,3,4,2};
	auto output = mysol.distinctDifferenceArray(res);
	printvector(output);
