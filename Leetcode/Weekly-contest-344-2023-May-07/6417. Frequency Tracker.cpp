...
public:
    FrequencyTracker() {
        FrequencyMap = vector<int>(10001,0);
        FrequencyCounts = vector<int>(10001,0);
    }
    
    void add(int number) { 
       FrequencyMap[number]++;
       FrequencyCounts[FrequencyMap[number]-1]--;
       FrequencyCounts[FrequencyMap[number]]++;
    }
    
    void deleteOne(int number) {
    	if(FrequencyMap[number]>0){
        	FrequencyMap[number]--;
        	FrequencyCounts[FrequencyMap[number]+1]--;
        	if(FrequencyMap[number]>0)
        		FrequencyCounts[FrequencyMap[number]]++;
