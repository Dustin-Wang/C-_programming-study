...
public:
    vector<int> colorTheArray(int n, vector<vector<int>>& queries){
        ...
        for(int i=0; i<NoQueries; i++)
        {
        	...
        	if(newColor != colors[loc])
        	{
        		//cout<<"queries[0]:"<<queries[0]<<" queries[1]:"<<queries[1]<<endl;//never print in leetcode.
        		if(loc+1<n){
        			if(colors[loc+1]==colors[loc] && colors[loc]!=0)
        				count--;
        			else if(newColor==colors[loc+1] && newColor!=0)
        				count++;
        		}
        		if(loc-1>=0 && colors[loc-1]!=0){
        			if(colors[loc-1]==colors[loc])
        				count--;
        			else if(newColor==colors[loc-1]&& newColor!=0)
        				count++;
        		}
