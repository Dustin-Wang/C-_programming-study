...
public:
    int minSessions(vector<int>& tasks, int sessionTime) {
    	...
    	//tasks.sort();
    	vector<vector<int>> possibleSumGrid(taskSize,vector<int>(sessionTime+1,0));
    	int largestPossibleSum = 0;
    	
    	for(int k = 1; k<= taskSize ;k++){
    		for(int x=0; x<=sessionTime;x++){
    			if(x-tasks[k]>=0) 
    				possibleSumGrid[x][k] |= possibleSumGrid[x-tasks[k]][k-1];
    			//possibleSum[x][k] |= possibleSum[x][k-1];
    			if(possibleSumGrid[x][k] && )
    				largestPossibleSum=max(largestPossibleSum, x);
    		}
    	}
...
int main(){
 ...
 //   	vector<vector<int>> mat = ArrayTo2dVector(array);
	...
	int k= 3;
	cout<<mysol.kthLargestNumber(nums,k)<<endl;
