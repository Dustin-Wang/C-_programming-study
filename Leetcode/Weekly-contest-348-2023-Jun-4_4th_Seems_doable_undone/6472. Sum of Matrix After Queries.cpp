...
public:
    long long matrixSumQueries(int n, vector<vector<int>>& queries) {
        vector<int> ColumnOccupied(n,0);
        long long ColumnUnOccupied = n;
        vector<int> RowOccupied(n,0);
        long long RowUnOccupied = n;
        ...
        for(int i=queriesSize-1; i>=0; i--)
        {
        	if(queries[i][0]==0)
			{
				int rowToChange = queries[i][1];
				if(RowOccupied[rowToChange])
					continue;
				else
				{
					// long long temp = 0;
					// for(int i=0; i<n; i++)
					// 	if(ColumnOccupied[i]==0)
					// 		temp++;
					res = res + ColumnUnOccupied*queries[i][2];
					RowOccupied[rowToChange]=1;
					RowUnOccupied--;
				}
			}
			else
			{
				int colToChange = queries[i][1];
				if(ColumnOccupied[colToChange])
					continue;
				else
				{
					// long long temp = 0;
					...
					// 	if(RowOccupied[i]==0)
					// 		temp++;
					res = res + RowUnOccupied*queries[i][2];
					ColumnOccupied[colToChange]=1;
					ColumnUnOccupied--;
				}
			}    
...
int main(){
	...
	cout<<mysol.matrixSumQueries(n, queries)<<endl;
