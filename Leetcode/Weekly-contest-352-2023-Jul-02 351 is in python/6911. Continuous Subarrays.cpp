...
public:
    long long continuousSubarrays(vector<int>& nums) {
        ...
        vector<int> Neighbors(5,-1);//2 is for counting the nums[i], 0,1,2,3,4 means nums[i]-2, nums[i]-1, nums[i], nums[i]+1, nums[i]+2
        Neighbors[2]=0;
        //for the longest subarray satisfying the condition and end at index i, we count the cloest index to i's left, including i in this subarray whose value
        //that is equal to nums[i]-2, nums[i]-1, nums[i], nums[i]+1, nums[i]+2

        long long res=1;
        int LengthOfLongestSubarrayEndAtJ= 1;
        int LengthOfLongestSubarrayStart = 0;
        for(int i=1; i< numsSize; i++)
        {
        	vector<int> NextNeighbors(5,-1);
        	NextNeighbors[2]=i;
        	if(abs(nums[i]-nums[i-1])>2)
        		LengthOfLongestSubarrayStart = i;
        	else if(nums[i]==nums[i-1]-2){
        		NextNeighbors[2]=i;
        		NextNeighbors[3]=Neighbors[1];//x-2+1 = x-1
        		NextNeighbors[4]=i-1;//x-2+2= x
        		LengthOfLongestSubarrayStart = max(LengthOfLongestSubarrayStart, Neighbors[3]+1);
        	}
        		...
        		// }
        		NextNeighbors[1]=Neighbors[0];//x-1-1 = x-2
        		NextNeighbors[2]=i;//x-1
        		NextNeighbors[3]=i-1;//x-1+1
        		NextNeighbors[4]=Neighbors[3];//x-1+2=x+1
        		LengthOfLongestSubarrayStart = max(LengthOfLongestSubarrayStart, Neighbors[4]+1);
        		// if(nums[i]==64)
        		// {
        		// 	printf("hello2\n");
        		// 	fflush(stdout);
        		// 	printvector(NextNeighbors);
        		// }
        	}
        	else if(nums[i]==nums[i-1]){
        		NextNeighbors[0]=Neighbors[0];
        		NextNeighbors[1]=Neighbors[1];
        		NextNeighbors[2]=i;
        		NextNeighbors[3]=Neighbors[3];	
        		NextNeighbors[4]=Neighbors[4];	
        	}
        	else if(nums[i]==nums[i-1]+1){
        		NextNeighbors[0]=Neighbors[1];//x+1-2= x-1
        		NextNeighbors[1]=i-1;//x+1-1=x
        		NextNeighbors[2]=i;
        		NextNeighbors[3]=Neighbors[4];//x+1+2=x+3
        		LengthOfLongestSubarrayStart = max(LengthOfLongestSubarrayStart, Neighbors[0]+1);
        	}
        	else if(nums[i]==nums[i-1]+2){
        		NextNeighbors[0]=i+2;//x+2-2=x
        		NextNeighbors[1]=Neighbors[3];
        		NextNeighbors[2]=i;
        		LengthOfLongestSubarrayStart = max(LengthOfLongestSubarrayStart, Neighbors[1]+1);
        	}
        	Neighbors = NextNeighbors;
        	// printvector(Neighbors);
        	// printf("i: %d LengthOfLongestSubarrayStart: %d\n",i, LengthOfLongestSubarrayStart);
        	
        	LengthOfLongestSubarrayEndAtJ = i-LengthOfLongestSubarrayStart+1;
        	res += LengthOfLongestSubarrayEndAtJ;
        }
        return(res);
    }
private:
...
int main(){
	...
	cout<<mysol.continuousSubarrays(nums)<<endl;
