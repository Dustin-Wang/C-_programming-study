...
public:
    int longestAlternatingSubarray(vector<int>& nums, int threshold) {
    	...
    	vector<int> LongestSubarrayEndAtJ(nums.size(),0);
    	if(nums[0]%2==0 && nums[0]<= threshold)
    		LongestSubarrayEndAtJ[0]=1;
        for(int i=1; i< numsSize; i++)
        {
        	if(nums[i]>threshold)
        		continue;
        	if(nums[i-1]<= threshold && nums[i]%2 != nums[i-1]%2)
        		LongestSubarrayEndAtJ[i]=LongestSubarrayEndAtJ[i-1]+1;
        	else if(nums[i]%2==0)
        		LongestSubarrayEndAtJ[i]=1;
        }
        // printvector(LongestSubarrayEndAtJ);
        for(int i=0; i< numsSize; i++)
        {
        	if(i<numsSize-1 && LongestSubarrayEndAtJ[i]!=0)
        		res = max(res, LongestSubarrayEndAtJ[i]);
        	else if(i==numsSize-1 && LongestSubarrayEndAtJ[i]!=0)
        		res = max(res, LongestSubarrayEndAtJ[i]);
        }
        return(res);
    }
...
int main(){
	...
	int threshold = 4;
	cout<<mysol.longestAlternatingSubarray(nums, threshold)<<endl;
