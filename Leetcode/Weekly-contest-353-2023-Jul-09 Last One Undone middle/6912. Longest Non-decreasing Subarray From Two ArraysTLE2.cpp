...
public:
    int maxNonDecreasingLength(vector<int>& nums1, vector<int>& nums2) {
        ...
        int res = 1;
        vector<int> LastValueForSubarrayLengthEndAtPrev(n,-1);
        LastValueForSubarrayLengthEndAtPrev[0]=min(nums1[0], nums2[0]);
        for(int i=1; i<n; i++)
        {
        	
        	for(int j=i; j>=1; j--)
        	{
        		if(LastValueForSubarrayLengthEndAtPrev[j-1]!=-1){
        			// printf("nums1[%d]:%d; nums2[%d]: %d; LastValueForSubarrayLengthEndAtPrev[%d-1]: %d\n", i, nums1[i], i, nums2[i], j, LastValueForSubarrayLengthEndAtPrev[j-1] );
        			if(min(nums1[i], nums2[i])>=LastValueForSubarrayLengthEndAtPrev[j-1]){
        				LastValueForSubarrayLengthEndAtPrev[j]=min(nums1[i], nums2[i]); 	
        				res = max(res, j+1);
        			}
        			else if(max(nums1[i], nums2[i])>=LastValueForSubarrayLengthEndAtPrev[j-1]){
        				LastValueForSubarrayLengthEndAtPrev[j]=max(nums1[i], nums2[i]); 	
        				res = max(res, j+1);
        			}
        			else
        				LastValueForSubarrayLengthEndAtPrev[j]=-1;
        					
        		}
        			
        	}
        	LastValueForSubarrayLengthEndAtPrev[0]=min(nums1[i], nums2[i]);
        	// printvector(LastValueForSubarrayLengthEndAtPrev);
        	// LastValueForSubarrayLengthEndAtPrev = LastValueForSubarrayLengthEndAtCur;
        }
        return(res);
    }
private:
