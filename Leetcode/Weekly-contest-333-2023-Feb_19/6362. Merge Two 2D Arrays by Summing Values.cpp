...
public:
    vector<vector<int>> mergeArrays(vector<vector<int>>& nums1, vector<vector<int>>& nums2) {
        map<int,int> numsValue;
        vector<vector<int>> res;
        for(auto num1: nums1)
        	numsValue[num1[0]] = num1[1];
        for(auto num2: nums2){
        	if(numsValue.find(num2[0])==numsValue.end())
        		numsValue[num2[0]] = num2[1];
        	else
        		numsValue[num2[0]] += num2[1];
        }
        for(auto iter= numsValue.begin(); iter!=numsValue.end(); iter++)
