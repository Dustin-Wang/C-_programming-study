...
public:
    int minimumIndex(vector<int>& nums) {
        ...
        {
        	if(it.second.size()*2>numsLen)
        		Dom = it.first;
        }
        // printf("Dom: %d\n", Dom);
        vector<int> LocsOfDom = Freq[Dom];

        int FreqDom = LocsOfDom.size();
        // printvector(LocsOfDom);
        // int lb = 0;
        // int ub = FreqDom-1;
        int res=numsLen;
        for(int mid = 0; mid< FreqDom; mid++)
        {
        	
        	// printf("lb: %d; ub:%d; mid: %d\n", lb, ub, mid);
        	//test whether this is a valid split
        	//for the subarray [0:LocsOfDom[mid]]
        	//the freq of dominant = mid+1;, and length of whole subarray is LocsOfDom[mid]+1
        	// printf("the freq of dominant = %d+1;, and length of whole subarray is LocsOfDom[%d]+1=%d\n", mid, mid, LocsOfDom[mid]+1);
        	//for the subarray [LocsOfDom[mid]+1: numsLen-1]
        	//the freq of dominant = FreqDom - (mid+1); and the length of whole subarray is numsLen-LocsOfDom[mid]-1
        	// printf("the freq of dominant = %d - (%d+1); and the length of whole subarray is %d-LocsOfDom[%d]-1=%d\n", 
        	// 	FreqDom, mid, numsLen, mid, numsLen-LocsOfDom[mid]-1);
        	if( (2*(mid+1)>LocsOfDom[mid]+1) && 2*(FreqDom - (mid+1))>numsLen-LocsOfDom[mid]-1){	
        		res=min(res, LocsOfDom[mid]);
        		break;
        	}
...
int main(){
	...
	cout<<mysol.minimumIndex(nums)<<endl;
