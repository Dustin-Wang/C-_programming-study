...
public:
    int maximumBeauty(vector<int>& nums, int k) {
        ...
        {
        	LeftBoundaries.push_back(nums[i]-k);
        	RightBoundaries.push_back(nums[i]+k);
        }
        ...
        int candidate = 0;
        int score = 0;
        int BeyondThisRight = 0;
        int BeforeThisLeft = 0;
        for(int i=0; i<numsLen; i++)
        {
        	candidate = LeftBoundaries[i];
        	BeyondThisRight = lower_bound(RightBoundaries.begin(), RightBoundaries.end(), candidate)-RightBoundaries.begin();
        	BeforeThisLeft = upper_bound(LeftBoundaries.begin(), LeftBoundaries.end(), candidate)-LeftBoundaries.begin();
        	score = BeforeThisLeft - BeyondThisRight;
        	res = max(res, score);
        	// printf("Candidate: %d; BeyondthisRight: %d, BeforethisLeft: %d; Score: %d\n", candidate, BeyondThisRight, BeforeThisLeft, score);

        	candidate = RightBoundaries[i];
        	BeyondThisRight = lower_bound(RightBoundaries.begin(), RightBoundaries.end(), candidate)-RightBoundaries.begin();
        	BeforeThisLeft = upper_bound(LeftBoundaries.begin(), LeftBoundaries.end(), candidate)-LeftBoundaries.begin();
        	score = BeforeThisLeft - BeyondThisRight;
        	res = max(res, score);
...
int main(){
	...
	int k=10;
	cout<<mysol.maximumBeauty(nums, k)<<endl;
