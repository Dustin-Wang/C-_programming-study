...
public:
    int addMinimum(string word) {
     	...
     	// vector<int> ABCCount(3,0);
     	// for(int i=0;i<wordLen; i++)
     	// {
     	// 	ABCCount[word[i]-'a']++;
     	// }
     	// int MaxCount = max(ABCCount);
     	// printf("MaxCount:%d\n", MaxCount);
     	// vector<int> DynamicRow(3*MaxCount,0);
     	int res=0;
     	int PointerInOutput = 0;
     	for(int i=0; i<wordLen; i++){
     		int tmp = (word[i]-'a');
     		
     		while(PointerInOutput%3 !=tmp)
     		{	
     			PointerInOutput++;
     			res++;
     		}	
...
int main(){
	...
	// string word ="b";
	// string word ="aaa";
	cout<<mysol.addMinimum(word)<<endl;
