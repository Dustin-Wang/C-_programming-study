...
public:
    int maxIncreasingGroups(vector<int>& usageLimits) {
        vector<int> Candidates=usageLimits;
  		...
  		int res =0;
  		int groupSize = 1;
  		while(Candidates.size()>=groupSize)
  		{
  			res++;
  			...
  			{
  				int pull = Candidates[i];
  				if (pull > 1)
  					ToBePushed.push_back(pull-1);
  			}
  			//merge the 2 sorted array, To be PUshed and Candidates[i, end]
  			groupSize++;
  			
  			vector<int> NewCanddiates;

  			int j = 0;
  			while(j<ToBePushed.size() && i<Candidates.size())
  			{
  				if(ToBePushed[j]>=Candidates[i]){
  					NewCanddiates.push_back(ToBePushed[j]);
  					j++;
  				}
  				else{
  					NewCanddiates.push_back(Candidates[i]);
  					i++;
  				}
  			}
