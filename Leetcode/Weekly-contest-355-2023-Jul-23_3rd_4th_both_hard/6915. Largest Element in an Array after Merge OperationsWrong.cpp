...
public:
    long long maxArrayValue(vector<int>& nums) {
        ...
        while(i< numsLen)
        {
        	acc = nums[i];
        	j= i;
        	while(j<numsLen-1 && nums[j+1]>=nums[j]){
        		acc = acc + nums[j+1];
        		j++;
        	}
        	NewNums.push_back(acc);
...
int main(){
	...
	vector<int> nums{40,15,35,98,77,79,24,62,53,84,97,16,30,22,49};//781
	cout<<mysol.maxArrayValue(nums)<<endl;
