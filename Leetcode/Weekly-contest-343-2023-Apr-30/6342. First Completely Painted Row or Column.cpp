...
public:
    int firstCompleteIndex(vector<int>& arr, vector<vector<int>>& mat) {
        int m = mat.size();
        int n = mat[0].size();
        vector<int> arrIndexMap(m*n,0);
        for(int i=0; i<m*n; i++)
        	arrIndexMap[arr[i]-1]=i;
        int firstindex =INT_MAX;
        for(int i=0; i<m; i++)
		{
			//foreach row, find the first index that it got fully painted
			// vector<int> RowIndexPair;
			int thisRowLastPainted = -1;
			for(int j=0; j<n; j++)
				if(arrIndexMap[mat[i][j]-1]>thisRowLastPainted)
					thisRowLastPainted = arrIndexMap[mat[i][j]-1];
			if(thisRowLastPainted<firstindex)
				firstindex = thisRowLastPainted;
		}
		for(int j=0; j<n; j++)
		{
			//foreach Column, find the first index that it got fully painted
			// vector<int> RowIndexPair;
			int thisColumnLastPainted = -1;
			for(int i=0; i<m; i++)
				if(arrIndexMap[mat[i][j]-1]>thisColumnLastPainted)
					thisColumnLastPainted = arrIndexMap[mat[i][j]-1];
			if(thisColumnLastPainted<firstindex)
				firstindex = thisColumnLastPainted;
		}
		return(firstindex);        	
    }
private:	
...
int main(){
	...
	// int inputs[][3]={{3,2,5},{1,4,6},{8,7,9}};
	vector<int> arr{1,3,4,2};
	int inputs[][2]={{1,4},{2,3}};
	
	vector<vector<int>> mat = ArrayTo2dVector(inputs);
	cout<<mysol.firstCompleteIndex(arr, mat)<<endl;
