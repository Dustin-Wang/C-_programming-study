...
public:
    int maxMoves(vector<vector<int>>& grid) {
        int m  = grid.size();
        int n= grid[0].size();
        vector<int> MaxMoves(m,0);
        // cout<<n<<endl;
        //starting from the second right most column
        for(int j=n-2; j>=0; j--)
        {
        	vector<int> PreviousColMaxMoves(m, 0);
        	for(int i=0; i<m; i++)
        	{
        		PreviousColMaxMoves[i]=0;
        		if(grid[i][j]<grid[i][j+1])
        			PreviousColMaxMoves[i]=max(PreviousColMaxMoves[i], MaxMoves[i]+1);
        		if(i-1>=0 && grid[i][j]<grid[i-1][j+1])
        			PreviousColMaxMoves[i]=max(PreviousColMaxMoves[i], MaxMoves[i-1]+1);
        		if(i+1<m && grid[i][j]<grid[i+1][j+1])
        			PreviousColMaxMoves[i]=max(PreviousColMaxMoves[i], MaxMoves[i+1]+1);
        	}
        	MaxMoves = PreviousColMaxMoves;
        	// printvector(MaxMoves);
        }
        int res = 0;
        for(int j=0; j<m; j++)
        	res = max(res, MaxMoves[j]);
        return(res);
    }
private:
