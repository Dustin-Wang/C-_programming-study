...
public:
    vector<vector<int>> findMatrix(vector<int>& nums) {
    	 map<int,int> CountOfNums;
    	 for(auto &i:nums)  
    	 {
    	 	if(CountOfNums.find(i)==CountOfNums.end())
    	 		CountOfNums[i]=1;
    	 	else
    	 		CountOfNums[i]++;
    	 }
    	 // vector<pair<int,int>> CountOfNums2;

		int MaxRows = 0;    	 
    	 for(auto &it:CountOfNums)
    	 	// CountOfNums2.push_back(pair<int,int>{it->second, it->first});
    	 	if(it.second > MaxRows)
    	 		MaxRows = it.second;    	 	
    	 
    	 
    	 vector<vector<int>> res(MaxRows, vector<int>());
    	 for(auto &it:CountOfNums)
    	 {
...
int main(){
	...
	vector<vector<int>> res = mysol.findMatrix(nums);
	for(auto &row:res)
		printvector(row);
