...
class Solution {
public:
    // vector<vector<int>> rangeAddQueries(int n, vector<vector<int>>& queries) {//time limit exceed
    ...
    // 				if(query[0]<=i && query[2]>=i && query[1]<=j && query[3]>=j)
    // 					res[i][j]++;
    // 			}		
    // 		}
    // 	}
    ...
    vector<vector<int>> rangeAddQueries(int n, vector<vector<int>>& queries) {//2 dimension to 1 dimension
    	vector<vector<int>> res;
    	for(int i=0;i<n; i++){
    		res.push_back(vector<int>(n,0));
    	};
    	vector<vector<int>> queryStartPointsForEachRow;
    	vector<vector<int>> queryEndPointsForEachRow;
    	for(int i=0; i<n; i++){
    		queryStartPointsForEachRow.push_back(vector<int>());
    		queryEndPointsForEachRow.push_back(vector<int>());
    	}
    	for(auto query: queries){
    		for(int i=query[0]; i<=query[2]; i++)
    		{
    			queryStartPointsForEachRow[i].push_back(query[1]);
    			queryEndPointsForEachRow[i].push_back(query[3]);
    		}
    	}
    	// for(int i=0;i<n;i++)
    	// {
    	// 	printvector(queryStartPointsForEachRow[i]);
    	// 	printvector(queryEndPointsForEachRow[i]);
    	// }

    	for(int i=0; i<n; i++){
    		sort(queryStartPointsForEachRow[i].begin(), queryStartPointsForEachRow[i].end());
    		sort(queryEndPointsForEachRow[i].begin(), queryEndPointsForEachRow[i].end());
			// printvector(queryStartPointsForEachRow[i]);
    		// printvector(queryEndPointsForEachRow[i]);	
    		for(int j=0; j<n; j++){
    			auto StartsPointsBeforeOrEqualJ = upper_bound(queryStartPointsForEachRow[i].begin(), queryStartPointsForEachRow[i].end(), j);
    			// printf("i:%d; j: %d; adding: %d\n", i,j, StartsPointsBeforeOrEqualJ - queryStartPointsForEachRow[i].begin());
    			res[i][j] +=StartsPointsBeforeOrEqualJ - queryStartPointsForEachRow[i].begin();

    			auto EndPointsBeforeJ = lower_bound(queryEndPointsForEachRow[i].begin(), queryEndPointsForEachRow[i].end(), j);
    			// printf("i:%d; j: %d; subtracing: %d\n", i,j, EndPointsBeforeJ - queryEndPointsForEachRow[i].begin());
    			res[i][j] -=EndPointsBeforeJ - queryEndPointsForEachRow[i].begin();
    		}
...
int main(){
	...
	queries.push_back(vector<int>{1,1,2,2});
	queries.push_back(vector<int>{0,0,1,1});

	vector<vector<int>> res= mysol.rangeAddQueries(n, queries);
	for(int i=0; i<n; i++){
		printvector(res[i]);
	}
