...
public:
    int maxNumOfMarkedIndices(vector<int>& nums) {
        ...
        {
        	mid = (lb+ub)/2;
        	//we use bisection method, check it is possible to find k pairs of marked points, k= mid
        	//if there are k pairs of marked points, then first k values of marked points, and second k values of marked points are marked.
        	//and pair (i, n-1-(k-i)) are paired, for i=0,...,k-1
        	int ExistsMidPairs = 1;
        	for(int i=0; i<mid; i++ )
        	{
        		if(2*nums[i]>nums[numsLen-(mid-i)])
        		{
        			ExistsMidPairs = 0;
        			break;
        		}
