...
public:
    long long minCost(vector<int>& nums, int x) {
    	...
    	vector<int> MinCostForType(numsSize,0);
    	long long res = 0;
    	for(int i=0; i<numsSize; i++){
    		res += nums[i];
    		MinCostForType[i]=nums[i];
    	}
    	unsigned long long newCost = 0;
    	for(unsigned long long k=1; k<numsSize; k++)
    	{
    		//assuming we are doing k rotations here
    		newCost = k*x;
    		for(int i=0; i<numsSize; i++){
    			MinCostForType[i] = min(MinCostForType[i], nums[(i+k)%numsSize]);
    			newCost += MinCostForType[i];
    		}
...
int main(){
	...
	vector<int> nums(1000,9);
	int x = 449844477;
	cout<<mysol.minCost(nums, x)<<endl;
