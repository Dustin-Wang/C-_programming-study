...
public:
    string smallestString(string s) {
    	...
    	int SecondA = -1;
    	for(int i=max(0,FirstNoA); i<sLen; i++)
    		if(s[i]=='a'){
    			SecondA = i;
    			break;
    		}
    	// cout<<"FirstA"<<FirstA<<endl;
    	// cout<<"FirstNoA"<<FirstNoA<<endl;
    	// cout<<"SecondA"<<SecondA<<endl;
		...
		for(int i=0; i<sLen;i++)
		{
			if(i<FirstNoA)
				res.push_back(s[i]);
			else if(i>=FirstNoA &&( SecondA == -1 || i<SecondA ))
				res.push_back(s[i]-1);
			else
				res.push_back(s[i]);
		}
...
int main(){
	...
	string s = "aaaaaa";
	cout<<mysol.smallestString(s)<<endl;
