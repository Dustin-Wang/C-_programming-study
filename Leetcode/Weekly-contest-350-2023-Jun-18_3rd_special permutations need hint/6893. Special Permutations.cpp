...
/*
6893. Special Permutations (quite difficult, when numslen is small, always use bit masking)
...
public:
    int specialPerm(vector<int>& nums) {
        ...
        vector<vector<int>> Neighbors(numsLen, vector<int>());
        ...
        int PickedMaxim = pow(2,numsLen);
        //since numslen <=14
        //we use a vector of vector to store the special permutations ending with nums[i]
        vector<vector<unsigned int>> specialPermutations;
        for(int i=0; i< numsLen; i++)
        {
        	specialPermutations.push_back(vector<unsigned int>(PickedMaxim,0));
        	specialPermutations[i][1<<i]=1;//this means the special permutations using no other nums and itself will be only 1 ways
        }

        //now we increase the numbders we use to construct the special permutations
        for(int i=2; i<=numsLen; i++)
        {
        	//still specialPermutations[i] is the special perumuations ending with nums[i]
        	 
        	for(int j=0; j< numsLen; j++)
        	{
        		for(const auto& Prev:Neighbors[j])
        		{
        			//how to list out the numbers the number of 1s in whose binary representations are exactly = i-1
        			for(int k=0; k<PickedMaxim; k++)//if k doesn't involve a 1 on jth bit, and no of 1s in k is i-1
        			{
        				// printf("for k= %d, i=%d; no of 1s: %d\n", k, i, __builtin_popcount(k));
        				if( (k&(1<<j)) ==0 && __builtin_popcount(k)==i-1){
        					specialPermutations[j][(k|(1<<j))]= (specialPermutations[j][(k|(1<<j))]+specialPermutations[Prev][k])%LargePrime;
        					// printf("k: %d; j: %d; Prev: %d\n",k,j, Prev );
        				}
        			}
        				
        		}
        		// printf("end with %d: %d\n", j, nums[j] );
        		// printvector(specialPermutations[j]);
        	}

        }
        int res = 0;
        for(int j=0; j< numsLen; j++)
        	res = (res+specialPermutations[j][PickedMaxim-1])%LargePrime;
        return(res);
