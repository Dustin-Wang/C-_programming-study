...
ou are given an array nums of positive integers and a positive integer k.
...
public:
    int beautifulSubsets(vector<int>& nums, int k) {
    	//at most 20 numbers
    	//let use  use integer to represent them
    	int n= nums.size();
    	...
    	// printf("hello\n");
    	// fflush(stdout);
    	vector<int> SubsetsUntilL{0};//0 meanes empty set
    	int res = 0;
    	int NewSubset = 0;
    	for(int l=0; l<n; l++)
    	{
    		//we are going to add nums[l] into the subsets we already have
    		//find whether there is nums[l]-k in previous array
    		vector<int> SubsetsUntilLplus1 = SubsetsUntilL;
    		if(HashMap.find(nums[l]-k)==HashMap.end())//then we can add
    		{
    			res = res+ SubsetsUntilL.size();

    			for(auto Subset: SubsetsUntilL)
    			{
    				NewSubset = Subset + (1<<l);
    				SubsetsUntilLplus1.push_back(NewSubset);
    			}
    		}
    		else
    		{
    			int WantToAvoid = HashMap[nums[l]-k];
    			for(auto Subset: SubsetsUntilL)
    			{
    				if((Subset &(WantToAvoid))==0)
    				{
    					NewSubset = Subset +(1<<l);
    					SubsetsUntilLplus1.push_back(NewSubset);
    					res++;
    		...
    		SubsetsUntilL=SubsetsUntilLplus1;
    	}
    	return(res);
    }
private:	
...
int main(){
	...
	int k=2;
	cout<<mysol.beautifulSubsets(nums, k)<<endl; 
