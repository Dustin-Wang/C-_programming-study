...
public:
    vector<vector<int>> sortTheStudents(vector<vector<int>>& score, int k) {
     	int StudentsNo = score.size();
     	int ExamNos = score[0].size();
     	vector<vector<int>> KthExamScores;
     	for(int i=0; i<StudentsNo; i++){
     		KthExamScores.push_back(vector<int>{score[i][k], i});
     	}
     	sort(KthExamScores.begin(),KthExamScores.end());//this is in ascending order
     	vector<vector<int>> res;
     	for(int i=StudentsNo-1; i>=0; i--){
     		int Student = KthExamScores[i][1];
     		res.push_back(score[Student]);
     	}
     	return(res);
