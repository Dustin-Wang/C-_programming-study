...
public:
    int minOperations(int n) {
     	int res = 0;
     	int consecurveones = 0;
     	int consecurvezeros = 0;
     	// int toBeAdded = 0;
     	vector<vector<int>> ConsecutiveDigits;//each entry {0, length of consecutive blocks}, {1, length of consecutive blocks}
     	for(int i=0; i<32; i++){
     		if(n & (1<<i)){
     			consecurveones++;
     			if(consecurvezeros >0 ){
     				ConsecutiveDigits.push_back(vector<int>{0, consecurvezeros});
     				consecurvezeros = 0;
     			}
     		}
     		else{//go back to 0
     			consecurvezeros++;
     			if(consecurveones >0){
     				ConsecutiveDigits.push_back(vector<int>{1, consecurveones});
     				consecurveones = 0;
     			}
     		}
     	}
     	ConsecutiveDigits.push_back(vector<int>{0, consecurvezeros});
     	for(auto block:ConsecutiveDigits)
     		printvector(block);
     	int NoOfBlocks = ConsecutiveDigits.size();
     	int accumulatedones = 0;
     	
     		for(int i=0; i<NoOfBlocks; i++){
     			if(ConsecutiveDigits[i][0]==0 && i!=0)
     			{
     				if( ConsecutiveDigits[i][1] ==1 ){
     					if((ConsecutiveDigits[i-1][1] + accumulatedones) > 1)
     						accumulatedones = 1;
     					else
     						accumulatedones = 0;
     					res +=1;
...
int main(){
	...
	// }
  	int n = 701;
  	cout<<mysol.minOperations(n)<<endl;
 	return(0);	
}