...
public:
    int maxNumOfMarkedIndices(vector<int>& nums) {
        ...
        map<vector<int>,int> EdgesBetweenPartiesPicked;
        
        //exposed means that this index has been marked
        //we are going to use the Berge's lemma, matching M is maximum iff there is M-augmenting path in G
        //M-augmenting path means a path that starts and ends on free (unmatched) vertices, and alternates between edges in and not in the matching
        for(int i=0; i< numsLen; i++){
        	...
        	for(int j=i+1; j< numsLen; j++){
        		if(2* nums[i] <= nums[j] || 2*nums[j]<= nums[i]){
        			...
        			vector<int> key1{i, j};
        			EdgesBetweenPartiesPicked[key1] = 0;	
        			vector<int> key2{j, i};
        			EdgesBetweenPartiesPicked[key2] = 0;	
        ...
        //check whether there is any M-augmenting path
        int IsThereMaugmenting = 1;
        vector<int> numsExposed(numsLen,0);
        int i = 0;
        while(i<numsLen)
        {
        	//vertex v is exposed if no edge of matching M is incident with v
        	// printf("numsExposed:\n");
        	// printvector(numsExposed);
        	
        	if(numsExposed[i]==0){
        		//check whether is a M-augmenting path from this unexposed point
        		vector<int> MPath;
        		vector<int> numsVisited(numsLen,0);
        		IsThereMaugmenting = DFSMAugmentPathExists(i, Neighbors, numsExposed, EdgesBetweenPartiesPicked, numsVisited, MPath);
        		// printf("MPath!:%d: IsThereMaugmenting:%d\n",i, IsThereMaugmenting);
        		// printvector(MPath);
        		if(IsThereMaugmenting)
        		{
        			//we will use MPath to modify our M
        			int MPathSize = MPath.size();
        			for(int k=0; k< MPathSize; k++)
        			{
        				numsExposed[k]=1;
        				if(k<MPathSize-1 && k%2==0 )
        				{
        					vector<int> key1{MPath[k], MPath[k+1]};
        					EdgesBetweenPartiesPicked[key1] = 1;	
        					vector<int> key2{MPath[k+1], MPath[k]};
        					EdgesBetweenPartiesPicked[key2] = 1;	
        				}
        				else if(k<MPathSize-1 && k%2==1)
        				{
        					vector<int> key1{MPath[k], MPath[k+1]};
        					EdgesBetweenPartiesPicked[key1] = 0;	
        					vector<int> key2{MPath[k+1], MPath[k]};
        			...
        			i=0;//reset our search region to 0
        			continue;
        		}
        		else
        			i++;
        ...
        int res = 0;
        for(auto it:numsExposed)
        	if(it==1)
        		res++;
        return(res);
    }
private:
	//if there is M-augmenting path from this start point, then change this path, and return 1 here
	//otherwise return 0
	int DFSMAugmentPathExists(int Start, vector<vector<int>>& Neighbors, vector<int>& numsExposed, map<vector<int>,int>& EdgesBetweenPartiesPicked,
			vector<int>& numsVisited, vector<int>& MPath)
	{
		numsVisited[Start]=1;
		MPath.push_back(Start);
		int NextEdgeInM = (MPath.size()+1)%2;//if this is 1, then next edge needs to be in M, if this is 0, then next edge needs to be out of M
		for(auto NextNode:Neighbors[Start])
		{
			vector<int> edge{Start,NextNode};
			if( EdgesBetweenPartiesPicked[edge] == NextEdgeInM )
			{
				if(numsVisited[NextNode]==0 )
				{
					//then we should go check this edge
					if(numsExposed[NextNode]==0)
					{
						MPath.push_back(NextNode);
						return(1);//we got one augmenting path
					}
					else
					{
						int temp = DFSMAugmentPathExists(NextNode, Neighbors, numsExposed, EdgesBetweenPartiesPicked, numsVisited, MPath);
						if(temp == 1)
							return(1);
					}
				}	
		...
		MPath.pop_back();
		return(0);//this means that we didn't find a M augmenting path here.
	}
