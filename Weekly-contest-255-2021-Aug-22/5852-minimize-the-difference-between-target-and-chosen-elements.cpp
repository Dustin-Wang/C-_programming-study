...
private:
	int res = INT_MAX;
	int target = 0;
	void DFS(vector<vector<int>>& mat, int level, int partialSum){
		...
		if(level <= Height -1 ){
			if( partialSum < target ){
				for(int k=0; k<Width; k++){
					//cout<<partialSum<<" "<<target<<endl;
					DFS(mat, level+1, partialSum + mat[level][k]);
			    }
			}
		}
		else if(level == Height ){
			//cout<<partialSum<<" "<<target<<endl;
			this->res = min(this->res, abs( partialSum- this->target ) );
		}
