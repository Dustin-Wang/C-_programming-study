...
public:
    int miceAndCheese(vector<int>& reward1, vector<int>& reward2, int k) {
        vector<pair<int,int>> rewardDiffs;
        int NoOfTypes = reward1.size();
        for(int i=0; i< NoOfTypes; i++)
        	rewardDiffs.push_back(pair<int,int>{reward1[i]-reward2[i], i});
        
        sort(rewardDiffs.begin(), rewardDiffs.end(),
        	[ ]( const pair<int,int>& lhs, const pair<int,int>& rhs )
			{
   				return lhs.first > rhs.first;
			}
        	);
        //we pick the first k types in rewardDiffs for mice 1
        int TypeForMice = 0;
        int res = 0;
        for(int i=0; i<NoOfTypes;i++)
        {
        	TypeForMice = rewardDiffs[i].second;
        	if(i<k)	
        		res = res + reward1[TypeForMice];	
        	else
        		res = res + reward2[TypeForMice];
        }
        return(res);
    }
private:	
...
int main(){
	...
	int k= 2;
	cout<<mysol.miceAndCheese(reward1, reward2, k)<<endl;
