...
public:
    int findTheLongestBalancedSubstring(string s) {
    	...
    	int res = 0;
    	int i=0;
    	int NoOfConsecutiveZeros = 0;
    	int NoOfConsecutiveOnes = 0;
    	while(s[i]=='1')
    		i++;
        if(i>0)
        	i--;
    	...
    	while(i< slen)
    	{	
    		NoOfConsecutiveZeros=0;
    		NoOfConsecutiveOnes = 0;
    		while(s[i]=='0'){
    			NoOfConsecutiveZeros++;
    			i++;
    		}
    		while(s[i]=='1'){
    			NoOfConsecutiveOnes++;
    		...
    		if(NoOfConsecutiveOnes>0)
    			i--;
    		res = max(res, 2*min(NoOfConsecutiveOnes, NoOfConsecutiveZeros));
    		i++;
    	}
    	return(res);
    }
...
int main(){
	...
	cout<<mysol.findTheLongestBalancedSubstring(s)<<endl; 
