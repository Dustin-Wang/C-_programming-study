...
/*
ou are given two 0-indexed strings word1 and word2.
...
public:
    bool isItPossible(string word1, string word2) {
        ...
        map<int,int> word1Collect;
       
        for(int i=0; i< word1Size; i++){
        	int key = word1[i]-'a';
        	if(word1Collect.find(key)==word1Collect.end())
        		word1Collect[key] = 1;
        	else
        		word1Collect[key]++;
        }
        int word1DistinctLetters = word1Collect.size();

         map<int,int> word2Collect;

        for(int i=0; i< word2Size; i++){
        	int key = word2[i]-'a';
        	// printf("%c ", word2[i]);
        	if(word2Collect.find(key)==word2Collect.end())
        		word2Collect[key] = 1;
        	else
        		word2Collect[key]++;
        }
        int word2DistinctLetters = word2Collect.size();


        
        
        for(auto candidate1:word1Collect)
        	for(auto candidate2:word2Collect){
        		int word1DistinctLettersAfter = word1DistinctLetters;
        		int word2DistinctLettersAfter = word2DistinctLetters;
        // printf("word1DistinctLettersAfter: %d\n", word1DistinctLettersAfter);
        // printf("word2DistinctLettersAfter: %d\n\n", word2DistinctLettersAfter);
        		//assume we swap candidate1 and candidate2
        		if(candidate1.second == 1)
        			word1DistinctLettersAfter--;
        			

        		if(word1Collect.find(candidate2.first) == word1Collect.end())
        			word1DistinctLettersAfter++;
        		else if(candidate2.first == candidate1.first && candidate1.second == 1)
        			word1DistinctLettersAfter++;
        			

        		if(candidate2.second ==1)
        			word2DistinctLettersAfter--;

        		if(word2Collect.find(candidate1.first)== word2Collect.end())
        			word2DistinctLettersAfter++;
        		else if(candidate1.first == candidate2.first && candidate2.second == 1)
        			word2DistinctLettersAfter++;

        		if(word1DistinctLettersAfter == word2DistinctLettersAfter){
        			// printf("candidate1.first: %c,candidate1.second: %d, word1DistinctLettersAfter: %d\n", candidate1.first+'a', candidate1.second, word1DistinctLettersAfter);
        			// printf("candidate2.first: %c,candidate2.second: %d,  word2DistinctLettersAfter: %d\n\n", candidate2.first+'a',candidate2.second, word2DistinctLettersAfter);
        			return(true);
        		}
...
int main(){
	...
	cout<<mysol.isItPossible(word1, word2)<<endl;
