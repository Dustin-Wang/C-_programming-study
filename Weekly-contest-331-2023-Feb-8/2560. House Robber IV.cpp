...
public:
    int minCapability(vector<int>& nums, int k) {
        ...
        vector<vector<int>> numsWithIndex;
        
        for(int i=0; i<numsLen; i++)
			numsWithIndex.push_back(vector<int>{nums[i],i});
			
		sort(numsWithIndex.begin(), numsWithIndex.end());		     	

		int LowerBound = numsWithIndex[0][0];
		int UpperBound = numsWithIndex[numsLen-1][0];
        ...
        while(LowerBound <= UpperBound)
		{
			...
			int accumulatedHouseLessThanOrEqualToMid = 0;
			int MaxHousesToStealForThisMid = 0;
			for(int i=0; i<numsLen; i++)
			{
				if(nums[i]<= mid)
					accumulatedHouseLessThanOrEqualToMid++;
				else{
					MaxHousesToStealForThisMid += (accumulatedHouseLessThanOrEqualToMid+1)/2;
					accumulatedHouseLessThanOrEqualToMid=0;
				}
			}
			if(accumulatedHouseLessThanOrEqualToMid > 0)
				MaxHousesToStealForThisMid +=(accumulatedHouseLessThanOrEqualToMid+1)/2;
			// printf("mid:%d\n", mid);
			// printf("MaxHousesToStealForThisMid: %d\n", MaxHousesToStealForThisMid);
			if(MaxHousesToStealForThisMid >=k)
				UpperBound = mid-1;
			else
				LowerBound = mid+1;
...
int main(){
	...
	// printvector(res);
  	...
  	// int k= 2;
  	cout<<mysol.minCapability(nums,k)<<endl;
