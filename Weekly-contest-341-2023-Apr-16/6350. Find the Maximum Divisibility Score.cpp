    ...
    int maxDivScore(vector<int>& nums, vector<int>& divisors) {
     	...
     	for(int i=0; i< divisorsLen; i++)
     	{
     		...
     		for(int k=0; k< numsLen; k++)
     			if(nums[k]%divisors[i]==0)
     				thisDivScore++;
     		// printf("i:%d, thisDivScore:%d\n",i,thisDivScore);
     		if(thisDivScore>MaxScore){
     			res = divisors[i];
     			MaxScore= thisDivScore;
     			// printf("res:%d\n",res);
     		}
     		else if(thisDivScore == MaxScore){
     			res = min(res, divisors[i]);
     			// printf("res:%d; divisors[%d]: %d\n",res,i,divisors[i]);
     		}
     	}
     	return(res);
    }
...
int main(){
	...
	cout<<mysol.maxDivScore(nums, divisors)<<endl;
