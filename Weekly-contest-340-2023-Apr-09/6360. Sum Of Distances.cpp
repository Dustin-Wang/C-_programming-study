...
public:
    vector<long long> distance(vector<int>& nums) {
     	...
     	{
     		int numValue = it.first;
     		vector<int> indices = it.second;
     		int indicesLen = indices.size();
     		long long temp = 0;
     		for(int i=1; i<indicesLen; i++)
     		{
     			//calculate the absolute distance to all indices on the left
     			temp = temp+ (indices[i]-indices[i-1])*i;
     			res[indices[i]] += temp;
     		}
     		temp =0 ;
     		for(int i=indicesLen-2; i>=0; i--)
     		{	
     			//calculate the absolute distance to all indices on the right
     			temp = temp+(indices[i+1]-indices[i])*(indicesLen-1-i);
     			res[indices[i]] += temp;
     		}
     	}
     	return(res);
...
int main(){
	...
	printvector(mysol.distance(nums));
